...
public class EnemySpawner : MonoBehaviour
{
    ...
    //reference to enemy prefabs... could reference in radar script to add a radar marker to each enemy in the list
    ...
    private void Awake()
    {
        for (int i = 0; 1 < EnemyPrefabs.Count; i++)
        {
    ...
    private void DoSpawnEnemy(int SpawnIndex)
    {
        ...
        if (poolableObject != null)
        {
            ...
            NavMeshHit Hit;                                                             // may need to change this value
            if (NavMesh.SamplePosition(Triangulation.vertices[VertexIndex], out Hit, 2f, -1))
            {
                enemy.Agent.Warp(Hit.position);
                // enemg needs to get enabled and start chasing now.
